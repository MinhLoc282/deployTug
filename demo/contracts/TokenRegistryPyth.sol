...
contract TokenRegistry is TokenRegistryInterface, Ownable {
    ...
    mapping(uint256 => bytes32) public priceIDList;

    // ------------------------- Errors ------------------------------------ //
    error SymbolAlreadyRegistered(string symbol);

    /// @inheritdoc TokenRegistryInterface
    function getPriceID(uint256 _index) external view override returns (bytes32) {
        return priceIDList[_index];
    }
    ...
    function registerToken(string calldata _symbol, bytes32 priceID)
        external
        onlyOwner
        returns (uint8 index)
    {
